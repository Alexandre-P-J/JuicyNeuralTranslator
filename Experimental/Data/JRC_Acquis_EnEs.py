...
class JRC_Acquis_EnEs_Dataset():
    class reader():
        def __init__(self, file):
            self.__file = file

        def translations(self):
            tmx_file = tmxfile(self.__file, "en", "es")
            for node in tmx_file.unit_iter():
                yield {"en": node.source, "es": node.target}

    def __init__(self):
        self.url = "https://object.pouta.csc.fi/OPUS-JRC-Acquis/v3.0/tmx/en-es.tmx.gz"
        home = Path.home()
        self.folder = home.joinpath(".cache/lemon/jrc-acquis/")
    ...
    def __download_if_required(self):
        ...
        if not os.path.isfile(self.data_filename):
            if not os.path.isfile(self.download_filename):
                print("Downloading JRC Acquis Dataset...")
